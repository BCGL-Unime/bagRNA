...
check_miniprot_fix() {
    if [ -f "working_dir/busco_fixed.gff" ]; then
...
if ! check_miniprot_fix; then
    agat_sp_fix_cds_phases.pl --gff tmp_dir/busco_genome.gff -f $input_fasta -o working_dir/busco_fixed.gff 2> logs/agat_sp_fix_cds_phases_busco_genome.log 
fi
...
# === Run Phobius ===
    if ! check_phobius; then
        phobius.pl final_struct_"$species"_"$strain"_prot.faa -short > output/phobius_"$species"_"$strain".txt 2> logs/phobius.log
    fi
